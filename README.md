...
<!-- 首图:录一段 login 扫码 + 下载的 GIF(转化率核心)。命令: node login.js && node bili_download.js <UID> -->
<!-- ![demo](docs/demo.gif) -->
...
node bili_download.js 508171633
node bili_download.js "https://space.bilibili.com/508171633/lists"

# 3b. 下载单个视频(自动识别 BV号)
node bili_download.js BV1U691YDEHz
node bili_download.js "https://www.bilibili.com/video/BV1U691YDEHz"
```
...
**反爬破三关**:① wbi 签名(Node `crypto` MD5) ② cookie(`login.js` 扫码拿 SESSDATA+buvid3) ③ 付费权限(cookie 解锁完整流)。

### 🕳️ 常见问题

- **付费视频只下到 3 分钟预览** → 扫码登录 `node login.js`(需账号已购)
- **合集下载报 -352 风控** → 缺 cookie,跑 `login.js`(单视频不受此限)
...
node login.js      # scan the QR with the Bilibili mobile app → cookies.txt
...
Node script orchestrates: input triage → Bilibili API (wbi-signed + cookie) for collection structure → spawns `yt-dlp` (stream + download) → `ffmpeg` (merge DASH). Beats three anti-scraping layers: wbi signature, cookie (SESSDATA+buvid3 via QR login), paid-content permission.
